...
function p._tenno (humanwords,size)
	...
	while j>0 do
		...
		elseif phonetic_list[j] == "v" then
            ...
            :css('display','inline-block')
            :css('background','blue')
            :css('transform','rotate(25deg) translateY(-20px)')				--add vowel translate in respect to size
			:css('transform-origin','top left')			--all rotate origin top left
            :css('margin','1px'):done()					--margin shrink in respect to size
            repeat
